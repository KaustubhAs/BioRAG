...
class TestQueryProcessor:
    ...
    def test_string_similarity_calculation(self, sample_graph):
        ...
        # Test exact match
        similarity = processor._calculate_string_similarity("diabetes", "diabetes")
        assert similarity > 0.8
        
        # Test substring match
        similarity = processor._calculate_string_similarity("diabet", "diabetes")
        assert similarity > 0.7
        
        # Test no match
        similarity = processor._calculate_string_similarity("diabetes", "hypertension")
...
class TestResponseGenerator:
    ...
    def test_context_extraction(self, sample_graph):
        ...
        context = generator.extract_context(diabetes_subgraph, "What are diabetes symptoms?")
        
        assert isinstance(context, list)
        assert len(context) > 0
        
        # Check if diabetes context is extracted
        diabetes_context = next((item for item in context if item['disease'] == 'Diabetes'), None)
    ...
    def test_context_formatting(self, sample_graph):
        ...
        context_list = [
            {'disease': 'Diabetes', 'symptoms': ['Fever', 'Fatigue']},
            {'disease': 'Hypertension', 'symptoms': ['Headache']}
        ]
