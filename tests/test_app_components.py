...
class TestCLI:
    ...
    def test_cli_run_with_query(self, mock_print, mock_input, sample_graph):
        ...
        mock_rag.answer_query.assert_called_once_with('What are diabetes symptoms?')
        assert mock_print.call_count >= 2  # At least query and response
    
    @pytest.mark.unit
    @patch('builtins.input')
    @patch('builtins.print')
    def test_cli_run_with_quit_command(self, mock_print, mock_input, sample_graph):
...
class TestStreamlitApp:
    ...
    def test_initialize_system_success(self, mock_rag, mock_build, mock_preprocess, sample_data):
        ...
        with patch.dict('app.streamlit_app.st.session_state', {}, clear=True):
            with patch('app.streamlit_app.st.spinner') as mock_spinner:
                with patch('app.streamlit_app.st.success') as mock_success:
                    ...
                    mock_success.assert_called_once_with("System initialized successfully!")
    ...
    def test_create_interactive_graph(self, sample_graph):
        ...
        graph_obj = create_interactive_graph(
            sample_graph, 
            show_diseases=True, 
            show_symptoms=True
        )
