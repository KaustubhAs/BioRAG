...
class TestDataProcessor:
    ...
    def test_preprocess_data_success(self, mock_dataset_path):
        ...
        assert len(symptoms) == 7  # 7 unique symptoms (duplicates removed by np.unique)
        assert len(relationships) == 9  # 9 relationships (3 diseases × 3 symptoms each)
...
class TestGraphBuilder:
    ...
    def test_build_graph_success(self, sample_data):
        ...
        for rel in sample_data['relationships']:
            ...
            assert graph.edges[rel['source'], rel['target']]['type'] == GraphSchema.HAS_SYMPTOM
    ...
    def test_build_graph_duplicate_relationships(self, sample_data):
        ...
        relationships_with_duplicates = sample_data['relationships'] + [
            {'source': 'Diabetes', 'target': 'Fever', 'type': 'HAS_SYMPTOM'}  # Duplicate
        ]
...
class TestGraphOperations:
    ...
    def test_node_label_filtering(self, sample_graph):
        ...
        disease_nodes = [n for n, attr in sample_graph.nodes(data=True) 
                        if attr.get('label') == GraphSchema.DISEASE]
        symptom_nodes = [n for n, attr in sample_graph.nodes(data=True) 
