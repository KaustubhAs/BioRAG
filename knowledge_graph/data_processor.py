...
def preprocess_data(data_path):
    ...
    symptoms = np.unique([s for s in all_symptoms if isinstance(s, str) and s.strip()])
    ...
    for _, row in df.iterrows():
        ...
        for col in symptom_columns:
            ...
            if not isinstance(symptom, pd.Series) and pd.notna(symptom) and str(symptom).strip():
