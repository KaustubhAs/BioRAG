...
# Page configuration
st.set_page_config(
    page_title="Biomedical Assistant",
    page_icon="🏥",
    layout="wide",
...
def create_interactive_graph(graph, show_diseases=True, show_symptoms=True, search_term=""):
    ...
    disease_nodes = [n for n, attr in graph.nodes(data=True) 
                     if attr.get('label') == "Disease"]
    symptom_nodes = [n for n, attr in graph.nodes(data=True) 
    ...
    if search_term:
        ...
        print(f"Search matched: {len(matched_diseases)} diseases, {len(matched_symptoms)} symptoms")
        print(f"Neighborhood includes: {len(disease_nodes)} diseases, {len(symptom_nodes)} symptoms")
    
    # Debug information (can be removed in production)
    print(f"Filter settings: show_diseases={show_diseases}, show_symptoms={show_symptoms}, search_term='{search_term}'")
    print(f"Visible nodes: {len(disease_nodes)} diseases, {len(symptom_nodes)} symptoms")
    ...
    for edge in graph.edges():
        if edge[0] in pos and edge[1] in pos and edge[0] in visible_nodes and edge[1] in visible_nodes:
...
def knowledge_graph_page():
    ...
    st.markdown("Explore the relationships between diseases and symptoms in our knowledge base.")
    ...
    if st.session_state.graph:
        ...
        disease_count = len([n for n, attr in graph.nodes(data=True) 
                           if attr.get('label') == "Disease"])
        symptom_count = len([n for n, attr in graph.nodes(data=True) 
                           if attr.get('label') == "Symptom"])
    ...
    with col1:
        if st.session_state.graph:
            ...
            if not show_diseases or not show_symptoms or search_term:
                ...
                if search_term:
                    filter_info.append(f"Search: '{search_term}' (showing matched nodes + neighbors)")
    ...
    with col2:
        ...
        **Interactions:**
        ...
        if st.session_state.graph:
            ...
            disease_nodes = [n for n, attr in st.session_state.graph.nodes(data=True) 
                           if attr.get('label') == "Disease"]
            for disease in disease_nodes[:10]:
                neighbor_count = len(list(st.session_state.graph.neighbors(disease)))
                st.write(f"• {disease} ({neighbor_count} symptoms)")

def qa_page():
    """Q&A Interface page."""
    st.title("🤖 Biomedical Q&A Assistant")
    st.markdown("Ask questions about diseases and symptoms. Get intelligent responses based on our knowledge base.")
        ...
        if st.button("Ask Question", type="primary"):
            if question.strip():
                with st.spinner("Processing your question..."):
                    try:
                        response = st.session_state.rag_system.answer_query(question)
    ...
    with col2:
        ...
        if st.session_state.graph:
            disease_count = len([n for n, attr in st.session_state.graph.nodes(data=True) 
                               if attr.get('label') == "Disease"])
            symptom_count = len([n for n, attr in st.session_state.graph.nodes(data=True) 
                               if attr.get('label') == "Symptom"])
        ...
        if search_history:
            filtered_history = [
                ...
                if search_history.lower() in item['question'].lower() or 
                   search_history.lower() in item['response'].lower()
            ]
