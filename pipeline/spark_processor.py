...
def preprocess_data(df):
    ...
    if 'Disease' in df.columns:
        ...
        logger.warning("Column 'Disease' not found in dataset. Partitioning may fail.")
...
def write_parquet_pandas(df, output_path):
    ...
    """
    logger.info("Converting to Pandas and writing Parquet (Windows-friendly path)...")
    pdf = df.toPandas()
...
def main():
    ...
    parser.add_argument("--input", type=str, default="data/dataset.csv", help="Path to raw CSV dataset")
    parser.add_argument("--output", type=str, default="data/processed/", help="Path to output Parquet directory")
    ...
    try:
        ...
        try:
            ...
            if isinstance(e, Py4JJavaError) or "HADOOP_HOME" in str(e) or "winutils" in str(e).lower():
                logger.warning("Spark write failed (Windows/HADOOP_HOME). Using Pandas/PyArrow for output.")
                write_parquet_pandas(df_clean, args.output)
                logger.info("Write operation completed successfully (Pandas/PyArrow).")
            else:
                raise
